...
class Number
{
     ...
     * 基于 BCMath 的高精度算术运算
     */
    public static function scale ( $scale=0 ){ return bcscale ($scale); }//设置所有bc数学函数的默认小数点保留位数
    public static function add( $left, $right, $scale=0 ){ return bcadd($left,$right,$scale); }//2个任意精度数字的加法计算
    public static function sub( $left, $right, $scale=0 ){ return bcsub($left,$right,$scale); }//2个任意精度数字的减法
    public static function mul( $left, $right, $scale=0 ){ return bcmul ($left,$right,$scale); }//2个任意精度数字乘法计算
    public static function power( $left, $right, $scale=0 ){ return bcpow ($left,$right,$scale); }//任意精度数字的乘方
    public static function div( $left, $right, $scale=0 ){ return bcdiv ($left,$right,$scale); }//2个任意精度的数字除法计算
    public static function mod( $left, $modulus ){ return bcmod ($left,$modulus); }//对一个任意精度数字取模
    public static function powmod( $left, $right, $modulus, $scale=0 ){ return bcpowmod($left,$right,$modulus,$scale); }//乘方取模
    public static function square( $left, $scale=0 ){ return bcsqrt  ($left,$scale); }//任意精度数字的二次方根
    public static function comp( $left, $right, $scale=0 ){ return bccomp ($left,$right,$scale); }//比较两个任意精度的数字
    ...
    public static function isFinite( $float ){ return is_finite($float); }//判断是否为有限值
    public static function isInfinite( $float ){ return is_infinite($float); }//判断是否为无限值
    ...
    public static function mtSrand( $seed=null ){ mt_srand($seed); }//播下一个更好的随机数发生器种子
    public static function mtGetrandmax(){ return mt_getrandmax(); }//显示随机数最大的可能值
     ...
     * 常用格式
     */
    public static function moneyFormat( $format, $number){ return money_format($format,$number); }//
    public static function numberFormat( $number, $decimals=0, $dec_point='.', $thousands_sep = ','){ return number_format($number,$decimals,$dec_point,$thousands_sep); }//以千位分隔符方式格式化一个数字
